...
def gyahun_dash_solution(pawns):

    def _getdiags(pawn):
        c, r = map(ord, pawn)
        return chr(c - 1) + chr(r - 1), chr(c + 1) + chr(r - 1)

    return len([p for p in pawns if any(d in pawns for d in _getdiags(p))])


def kerulen_solution(pawns):
    answer = 0
    for pawn in pawns:
