...
class GyahunDashFriends(set):
    def __init__(self, pairs=set()):
        super(GyahunDashFriends, self).__init__(map(frozenset, pairs))

    def add(self, pair):
        if pair in self: return False
        super(GyahunDashFriends, self).add(frozenset(pair))
        return True

    def remove(self, pair):
        if pair not in self: return False
        super(GyahunDashFriends, self).remove(pair)
        return True
    ...
    def connected(self, name):
        return GyahunDashFriends(filter({name}.issubset, self)).names() - {name}
